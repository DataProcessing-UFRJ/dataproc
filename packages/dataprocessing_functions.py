...
def acquisition_remove(dataset,
    ...
    if filename_filters is not None:
        ...
        filename_filtered = []
        for string_expr in filename_filters:
            filename_filtered += glob.glob(os.path.join(folder,string_expr), recursive=False)
...
def header_image(file, reset_header=False, instrument='SAMI'):
...
def header_init(header, instrument='SAMI'):
    ...
    header.remove("WCSASTRM", ignore_missing=True)
    header.remove("RADECEQ", ignore_missing=True)
    header.remove("WAT0_001", ignore_missing=True)
...
def reduce_image(image_file, 
    ...
    if master_bias is not None:
        if shared_memory:
            ...
            mbias_buf = [SharedMemory(name=master_bias[i]['id']) for i in range(len(master_bias))]
    ...
    if master_flat is not None:
        if shared_memory: 
            ...
            mflat_buf = [SharedMemory(name=master_flat[i]['id']) for i in range(len(master_flat))]
    ...
    for ne,ext in enumerate(image_exts):
        ...
        #.OVERSCAN correction
        if ('OVERSCAN' not in img.header):
            #.Goodman exception
            #.(do not have an overscan section)
            if inst.find('Goodman') >= 0:
                proc_string+="o"

                img.header['BITPIX']=-32
                img.header.remove('BZERO', ignore_missing=True)
                img.header.remove('BSCALE', ignore_missing=True)
                ...
                img.header.remove('BIASSEC', ignore_missing=True)
                img.header.remove('TRIMSEC', ignore_missing=True)
                img.header.remove('BZERO', ignore_missing=True)
                img.header.remove('BSCALE', ignore_missing=True)
...
def combine_flat(image_list, 
    ...
    if mflat[0].header['INSTRUME'].lower().find('goodman') >= 0:
    ...
    #..MEF exception (e.g. SOI, SAMI): creating merged flat-field
    if len(image_exts) > 1:
        copy2(output,output.replace('flat','flat_merged'))
        reduce_image(output.replace('flat','flat_merged'), merge_amplifiers=True)
